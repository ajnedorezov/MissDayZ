...
public class DriveTurnController extends SynchronousPID implements Controller {
	...
	private DriveTurnController() {
		...
		SmartDashboard.putNumber("DTC_kP", 0.1);
	...
	public synchronized void run() {
			...
			double turn = Math.signum(cmd)*Math.max(Math.abs(cmd), Inputs.Properties.PID_DRIVE_TURN_MIN_OUTPUT);
			//double turn = cmd;
			...
			if (Math.abs(error) < Inputs.Properties.PID_DRIVE_ANGLE_TOLERANCE)
				turn = 0.0;
			...
			/*
			if (turn < -mMinMotorOutput)
				turn = -mMinMotorOutput;
			else if (turn > mMinMotorOutput)
				turn = mMinMotorOutput; */
	...
	public synchronized boolean onTarget() {
		...
		return Math.abs(error) < Inputs.Properties.PID_DRIVE_ANGLE_TOLERANCE;
	}
	...
	public void loadProperties() {
		if (mPIDFromSmartDashboard) {
			...
			mMaxMotorOutput = mPropertySet.getDoubleValue("turnPIDMaxMotorOutput", Constants.Properties.PID_DRIVE_TURN_MAX_OUTPUT);
			mMinMotorOutput = mPropertySet.getDoubleValue("turnPIDMinMotorOutput", Constants.Properties.PID_DRIVE_TURN_MIN_OUTPUT);
			setPID(kp, ki, kd);
			*/	
		}	
	}
